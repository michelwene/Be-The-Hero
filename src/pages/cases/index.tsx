...
export default function Cases({ userData, caseData }: CaseProps) {
  const [cases, setCases] = useState<Case[]>(caseData);
  ...
  return (
    <Box as="section" maxWidth="78vw" mx="auto" my={["20px"]}>
      <Heading
        as="header"
        display="flex"
        justifyContent="space-between"
        alignItems="center"
        rowGap={["20px", "20px", "0px"]}
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  const getUserData = handleRecoverUserDataFromCookies(ctx);
  const apiServer = setupAPI(ctx);

  const { data } = await apiServer.get("/cases");
